...
function QuestionCard({ ques }: { ques: Models.Document }) {
    ...
    return (
        <div ref={ref} className="relative flex flex-col gap-4 overflow-hidden rounded-xl border border-white/20 bg-white/5 p-4 duration-200 hover:bg-white/10 sm:flex-row">
            <ShineBorder shineColor={["#A07CFE", "#FE8FB5", "#FFBE7B"]} duration={12} />
            <div className="relative shrink-0 text-sm sm:text-right">
                <p>{ques.totalVotes} votes</p>
                <p>{ques.totalAnswers} answers</p>
            </div>
            <div>
                <Link href={`/questions/${slugify(ques.title)}/${ques.$id}`} className="text-orange-500 duration-200 hover:text-orange-600">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                    ...
                    <span>asked {dayjs(ques.$createdAt).fromNow()}</span>
                </div>
            </div>
        </div>
    )
